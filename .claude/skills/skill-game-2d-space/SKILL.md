...
- 2026-09-15：每一局都是任務（四種類型 + 星塵礦點 + 事件在玩家附近 + 準星修正）。**絕地戰兵方向的下一步候選**：任務中的「增援」（陣亡隊友限次數空投復活）、~~任務簡報畫面~~（2026-09-15 已做：`#briefing`，`beginSolo` 先決定種子 / 類型 → `renderBriefing` → `launchPending`；多人房間尚無）、次要目標 / 可選任務、據點防禦塔、Boss 招式改成以據點為中心（現在流星 / 彈牆仍用地圖座標）、手把。
