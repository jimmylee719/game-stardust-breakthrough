...
- Boss 挑戰：`startRun({ bossRush: true })` → `world.mods.bossRush`；`nextWave` 每波都走 `startBossWave`，`spawnBoss` 在 bossRush 分支出 1–`min(6, 1 + wave/2)` 隻（`hpMul = 1/√n`、slot 從 -2.5..2.5 排開、最後一波含殲滅者），勝利波數 `BOSS_RUSH_WAVES`（8），獎勵 `BOSS_RUSH_REWARD`（1.5，killBoss 分數、伺服器 dust）。客戶端 `beginSolo(0, null, true)`、`submitRun({ boss })`；伺服器 start 訊息 `m.boss`；sim `--boss`。**不要再用「從第 4 波開始」當 Boss 挑戰。**
- 機體 / 武器相容：`SHIPS[].weapons`（白名單）或 `noWeapons`（黑名單），`weaponAllowed(ship, weapon)` / `defaultWeaponFor(ship)`，在 `addPlayer`、main.js `currentWeapon()`、hangar 卡片（`.incompatible`）、伺服器 join 四處都套。
...
- **所有 UI 入口都是任務**（2026-09-15 起）：`beginSolo()` / 大廳 start / 每日都走 `startRun({ mission: true, missionType, bossRush, seed })`；類型 `MISSION_TYPES`（relay / exterminate / nests / boss），目標物件 `world.objectives[].kind`（relay: progress/active；nest: eid + pending 由 killEnemy 設；boss: spawned → `spawnSiteBosses`；kill: count/need 由 killEnemy 累加），`world.caches` → 不消失的 dust pickups。Boss 錨點 `b.homeX/homeY`（updateBoss 用 `?? 舊值`）。危險 / 事件位置用 `nearPt(world, p, R)`；`spawnAmbient` 在任務地圖以隨機玩家為中心。波次引擎（nextWave / 特殊波 / 20 波通關）保留給測試與舊邏輯，UI 不再進。
- 探索任務：`startRun({ mission: true })` → `world.mods.mission`，`shared/map.js` 的 `MISSION / genMap(seed, arena) / resolveObstacles / obstacleAt / TERRAIN`；世界 `W/H` 改成 4800×2700，`world.obstacles / objectives / extract / start / mapSeed / missionT / alarm / patrolCd`。`update()` 在波次區塊前 `if (mission) { updateMission(); return; }`；生成走 `edgeSpawn → ringSpawn`（視野外環帶）。快照送 `map: {seed}` + `mods` + objectives / extract，`applySnapshot` 種子不同才重建地圖、沒有 map 就還原 1600×900。**render 的 `W/H` 是視窗，世界用 `world.W/H`**；攝影機 `cam`（`updateCamera`，一般模式恆為 0），世界層在 `ctx.translate(-cam.x, -cam.y)` 之後畫，視窗層（HUD、天候、公告）在 translate 回來之後；全畫面填色要用 `fillRect(cam.x, cam.y, W, H)`；`toWorld` 加 cam，main 每幀 `renderer.refreshMouse(mouse)`。剔除用 `vis(x, y, m)`。前景視差層 `fgFor(id)` 會把 `img/fg-<arena>.webp` 裁掉底部（`FG_CROP`，AI 常畫地平線）並羽化邊緣做成貼片，screen 混合拼貼。新 HUD 在 `drawMissionHUD`（含小地圖）。
- 隨機事件與場地危險一律無差別：太陽風暴（`F.hitE`）、火柱 / 冰刺、EMP（`z.hitE`）、毒區、壓力區都會打敵人與 Boss（原生屬性免疫），`EVENT_CD = [25, 40]`。
...
- 主武器：`WEAPONS`（blaster / flame / frost / arc / laser / blade / scatter / missile / venom，`WEAPON_STATS`）。散彈 = 短命 `b.pellet`（bounce → 壽命 ×2、bigshot → `b.knock`）；飛彈 = `b.rocket`（命中 / 到期 / 撞 Boss 都走 `explodeRocket`，`b.splash`，集束進化 `b.mini`）；毒液 = `b.glob` → `venomCloud` 生 zone `kind: 'venom'`（只毒敵人，`e.poison / poisonDps / poisonBy` 在敵人迴圈燃燒之後扣血；瘟疫進化在 `onEnemyDeath`）。子彈追蹤半徑用 `b.homingR`。**組合技對非子彈武器要有回退**（`emberProc`、beam 段 `wallMul`、arc 目標 +1、shock 僵直），新武器記得補。，發射邏輯在 `update()` 的「射擊：依主武器分派」；`p.weaponOn` 給連續武器渲染；閃電走 `fx.bolt(points)`；伺服器 join 用 `weaponUnlocked` 驗證，解鎖 id `weapon:<id>`。
...
- 機體：`SHIPS`（falcon / wasp / bastion / carrier / ronin / wraith / engineer / phoenix / reaper）；鳳凰 `flags.rebirth`（`playerDown` 開頭攔截 → `rebirth()`）、`fireTrail`（stepPlayer 衝刺中推 fire zone）、`fireBoost`；收割者 `flags.harvest` + `p.harvest`（`hitEnemy` 傷害、`stepPlayer` 速度、`killEnemy` 疊層、`hurtPlayer` 減半；predict FIELDS 含 harvest）。外觀差異：`SHIP_HULL / SHIP_FLAME` + render `shipAccent`，cardart 同步。**敵彈迴圈開頭要 `if (!b) continue`**（重生 / 擊殺會整批重建 enemyBullets）。，`applyShip` 在 `addPlayer` 內、永久強化之前套用；客戶端 join 帶 `ship`，伺服器用 `shipUnlocked(ship, unlocks)` 驗證後才建玩家（join 改成先 `await db.auth`）；解鎖走 `/api/perks/buy` 的 `ship:<id>`；`render.js` 的 `shipPath(ship)` 畫船身。
...
- 2026-09-10 美術二：首頁 Logo 徽章（`public/img/logo.webp`，黑底用 `mix-blend-mode: screen` 融入面板）、機庫卡圖機制（main.js `ART` 集合 + `artOf(kind, id)`，圖放 `public/img/<kind>-<id>.webp`，CSS `.ship .art` 用 `background-size: 140%` 裁掉生成圖的白邊）。目前只有 `ship-falcon`；**其餘 6 機體 / 6 武器 / 6 技能要等 Hugging Face ZeroGPU 每日額度重置再生**，prompt 要強調「no text, no border, full-bleed, dark navy background」避免模型自己加卡框與英文名。機庫依使用者決定改回單欄捲動（寬 760px），捲軸用細霓虹樣式（全站 `scrollbar-width: thin` + `::-webkit-scrollbar`）。
- 2026-09-23：任務結算面板 `#debrief`（`showDebrief` / `debriefReward` / `debriefAgain`；`stats.caches`、`reinforce0` 入快照）。
- 2026-09-15：每一局都是任務（四種類型 + 星塵礦點 + 事件在玩家附近 + 準星修正）。**絕地戰兵方向的下一步候選**：~~增援~~（2026-09-23 已做：`MISSION.reinforce` 2 + 每人 1，5 秒後空降、3 秒無敵，快照 `reinforce` / `respawnT`）、~~任務簡報畫面~~（2026-09-15 已做：`#briefing`，`beginSolo` 先決定種子 / 類型 → `renderBriefing` → `launchPending`；2026-09-23 多人也有：`brief` / `brief_cancel` / `start` 三段握手，`room.brief`，`onCoopBrief`）、~~次要目標~~（2026-09-23 已做：`SIDE_TYPES` 補給站 / 雷達站，`world.side`，各 +400）、~~據點防禦塔~~（2026-09-23 已做：`ENEMY_TYPES.turret` + `BEHAVE.turret`，`genMap().turrets`）、~~Boss 招式以據點為中心~~（2026-09-23 已做：`BOSS_LEASH`、任務彈牆繞 Boss、`bossMeteor`、太陽風暴 `flare.cy`）、手把。
- 2026-09-14：探索任務（大地圖 + 攝影機 + 地形 + 中繼站 / 撤離 + 小地圖）、動態視差環境（星雲、AI 前景層 fg-*.webp、塵埃、落影）。AI 圖層 prompt 要「pure solid black background, isolated objects, no horizon, no ground」，模型仍常畫地平線 → 用 `FG_CROP` 裁掉。**使用者想往絕地戰兵（Helldivers）方向走：任務型玩法、可探索、2D 為主頂多一點 2.5D**；下一步候選：更多任務類型（護送、清巢、Boss 據點）、地圖用 AI 貼圖、Boss 在任務地圖登場、敵人巢穴 / 據點。
- 2026-09-10 晚：Esc 暫停面板全螢幕鈕（`wasFullscreen`，繼續時自動回全螢幕）、事件 / 危險無差別、武器 ×3（scatter / missile / venom）+ 機體 ×2（phoenix / reaper）+ 機體外觀差異、組合技回退、SEO / AEO（head meta + JSON-LD + about 面板 + robots / sitemap / llms.txt，語言切換同步 title / description，`?lang=en` 直接英文）。
- 2026-09-10 後半：背景圖調淡（raw 0.6 + 漸層 0.7 + 暗色 0.42）、Boss 挑戰重做、機體武器相容、毒霧效果、冰凍 / 雷射 DPS +30%。sim 機器人不會走位也不會靠近（劍聖在 sim 永遠打不到），**用 sim 比機體強度只能看趨勢**，堡壘最耐、其餘在 2–5 波倒下屬正常。
- 2026-09-10 美術三：外部生圖都不可用時（HF ZeroGPU 額度全 Space 共用、Canva 團隊未開放生圖）改為 **程式即時畫卡圖**：`public/js/cardart.js`（`drawShipCard / drawWeaponCard / drawSkillCard / paintArt`），main.js 的 `artOf()` 在 `ART` 集合沒有圖檔時輸出 `<canvas class="art" data-art="kind:id">`，各 render 函式設完 innerHTML 後呼叫 `paintArt(container, skin)`。**全域 `canvas {}` 規則已改成 `canvas#game`**（否則卡圖 canvas 會被 position: fixed 撐滿全螢幕）。
- 毒霧（zone kind `fog`）原本只有畫面沒有效果：現在 `updateZones` 每 tick 先清 `e.fog / q.fog` 再標記霧內單位；`nearestPlayer / nearestTarget` 對霧內目標要貼近 `FOG_REVEAL`（160）才鎖得到；render 霧內敵人 alpha 0.12（玩家貼近 0.75），HUD 標籤「毒霧隱蔽」。**規則：任何新的區域 / 指示圖案都要在 test/syn.js 第 22 組加一條「真的有效果」的斷言。**
