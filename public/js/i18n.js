...
const EXACT = {
  ...
  // 敵人
  '蟲群': 'Swarmer', '蟲巢': 'Nest', '飛鏢': 'Dart', '重裝': 'Tank', '射手': 'Shooter', '分裂體': 'Splitter', '小行星': 'Asteroid', '護盾兵': 'Warden', '狙擊手': 'Sniper', '迫擊砲': 'Mortar', '自爆蟲': 'Kamikaze', '咒術師': 'Hexer', '虛空哨兵': 'Void sentinel', '餘燼': 'Ember', '脈衝體': 'Pulsar', '孢子母體': 'Spore mother', '燈籠魚': 'Angler', '霜噬': 'Frostbite', '雷射兵': 'Lancer', '流星': 'Meteor', '飛碟': 'UFO',
...
const RULES = [
  ...
  [/^威脅等級 (\d+)$/, (m) => `Threat level ${m[1]}`],
  [/^蟲巢 (\d+) ✔$/, (m) => `Nest ${m[1]} ✔`],
  [/^蟲巢 (\d+)$/, (m) => `Nest ${m[1]}`],
  [/^殲滅 (\d+) \/ (\d+)$/, (m) => `Kills ${m[1]} / ${m[2]}`],
  [/^拆除蟲巢 (\d+) \/ (\d+)$/, (m) => `Nests ${m[1]} / ${m[2]}`],
  [/^還剩 (\d+) 個目標$/, (m) => `${m[1]} objectives left`],
  [/^蟲巢摧毀 \+(\d+)$/, (m) => `Nest destroyed +${m[1]}`],
  [/^據點肅清 \+(\d+)$/, (m) => `Site cleared +${m[1]}`],
  [/^殲滅目標達成 \+(\d+)$/, (m) => `Kill target reached +${m[1]}`],
  [/^(\d+) 隻 Boss 據守此地！$/, (m) => `${m[1]} bosses hold this site!`],
  [/^任務完成：(.+) · 全員撤離 · 用時 (.+)$/, (m) => `Mission complete: ${tr(m[1])} · team extracted · time ${m[2]}`],
  [/^中繼站 (\d+) ✔$/, (m) => `Relay ${m[1]} ✔`],
  [/^中繼站 (\d+)$/, (m) => `Relay ${m[1]}`],
  [/^啟動中 (\d+)s$/, (m) => `Activating ${m[1]}s`],
  [/^啟動中繼站 (\d+) \/ (\d+)$/, (m) => `Relays ${m[1]} / ${m[2]}`],
  [/^還剩 (\d+) 座中繼站$/, (m) => `${m[1]} relays left`],
  [/^中繼站啟動 \+(\d+)$/, (m) => `Relay online +${m[1]}`],
  [/^撤離成功 \+(\d+)$/, (m) => `Extracted +${m[1]}`],
  [/^撤離點已標記，全員抵達後撐住 (\d+) 秒$/, (m) => `Extraction marked — everyone inside for ${m[1]} s`],
  [/^三座中繼站全部啟動，全員撤離 · 用時 (.+)$/, (m) => `All three relays online, team extracted · time ${m[1]}`],
...
Object.assign(EXACT, {
  ...
  '🧭 探索任務': '🧭 Expedition', '探索任務': 'Expedition', '一般波次': 'Standard waves', '🧭 單人出擊': '🧭 Solo sortie', '👑 Boss 挑戰': '👑 Boss rush', '任務類型': 'Mission type', '🎲 隨機': '🎲 Random', '📡 中繼站': '📡 Relays', '💀 殲滅': '💀 Exterminate', '🕳️ 拆巢': '🕳️ Nests', '👑 Boss 據點': '👑 Boss site', '👑👑 Boss 挑戰': '👑👑 Boss rush',
  '中繼站啟動': 'Relay activation', '殲滅': 'Exterminate', '拆除蟲巢': 'Destroy the nests', 'Boss 據點': 'Boss site', '蟲巢': 'Nest', '據點已肅清': 'Site cleared', '前往 Boss 據點': 'Head to the boss site', '擊破據點的 Boss': 'Destroy the site boss', '據點守衛出現！': 'Site guardian appears!', '目標完成！前往撤離點': 'Objectives complete! Head to extraction', '（Boss 挑戰：一次多隻，獎勵 ×1.5）': ' (Boss rush: several at once, rewards ×1.5)',
  '站在三座中繼站各 6 秒把它們啟動，啟動中敵人會加倍湧來': 'Stand inside each of the three relays for 6 s to activate them; enemies double while you do', '擊殺指定數量的敵人（敵人會不斷增援），達標後撤離': 'Kill the required number of enemies (they keep reinforcing), then extract', '找到三座會不斷生敵的蟲巢並摧毀它們': 'Find and destroy three nests that keep spawning enemies', '深入 Boss 盤據的據點，擊破後撤離；Boss 挑戰會一次出好幾隻': 'Push into the boss-held site, destroy it and extract; Boss rush spawns several at once',
  '任務目標：不會移動，每隔幾秒生出一批敵人，拆掉才會停': 'Objective: does not move, spawns a batch of enemies every few seconds until destroyed', '中繼站': 'Relay', '撤離點': 'Extraction', '撤離成功': 'Extracted', '前往撤離點': 'Head to the extraction point', '⚠ 警戒：敵人增援中': '⚠ Alert: reinforcements incoming',
...
<h3>Missions</h3><ul><li>Every run is a mission on a 3 × 3-screen map with a camera that follows you. Four mission types: 📡 Relay activation, 💀 Exterminate (kill a set number), 🕳️ Nests (destroy three spawners), 👑 Boss site (the boss appears when you approach; Boss rush spawns several). Eight stardust caches are scattered on every map. Terrain blocks ships and shots (use it for cover; ricochet shots bounce off it), enemies pour in from beyond your view and the threat level rises every 45 s. Activate three relays (stand inside each for 6 s — enemies double while you do), then the extraction point lights up: keep the whole team inside for 8 s to extract (+3000 score, ×1.3 stardust). The minimap (bottom right) shows objectives and teammates; gold edge arrows point to the nearest relay.</li></ul><h3>Arenas & elements</h3><ul><li>Six arenas, each with its own hazards and native creatures: Deep space (void rifts), Inferno (lava, fire geysers), Mercury (gravity wells, solar storms), Venom (drifting toxic tides), Abyss (drag, currents, pressure), Glacier (slippery ice, blizzards, ice spikes).</li><li>Every weapon has an element and every creature has an affinity: frost hurts fire creatures, fire hurts ice and toxic, lightning wrecks water creatures, kinetic rounds (blaster / blade) punch through plasma, lasers cut void. Super-effective hits show ×1.5+.</li></ul>
