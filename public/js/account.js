...
export async function ensureAccount(name) {
  try {
    ...
    if (acct && /unauthorized/.test(e.message)) { acct = null; localStorage.removeItem(KEY); return ensureAccount(name); }
...
export async function submitSoloRun(score, wave) {
  if (!acct || score <= 0) return null;
  try { return await api('/api/runs', { method: 'POST', body: JSON.stringify({ id: acct.id, secret: acct.secret, score, wave }) }); }
  catch (e) { console.warn('submit failed:', e.message); return null; }
}
