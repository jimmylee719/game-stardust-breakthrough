...
const FIELDS = ['x', 'y', 'vx', 'vy', 'dashing', 'dashCd', 'inv', 'r', 'speedMul', 'dashCdMax', 'color'];

export function createPredictor(world) {
  const st = { x: 0, y: 0, vx: 0, vy: 0, dashing: 0, dashCd: 0, inv: 0, r: 14, speedMul: 1, dashCdMax: 1.2, color: '#fff', angle: 0 };
  ...
  return {
    ...
    reconcile(serverMe) {
      ...
      // 以權威狀態為起點
      for (const f of ['x', 'y', 'vx', 'vy', 'dashing', 'dashCd', 'inv', 'speedMul', 'dashCdMax']) if (serverMe[f] !== undefined) st[f] = serverMe[f];
