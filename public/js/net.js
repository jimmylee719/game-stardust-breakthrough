...
export function connect({ name, code, token, acct, ship, weapon, skin, onWelcome, onLobby, onStarted, onError, onClose, onReconnecting, onResult }) {
  ...
  function open() {
    ...
    net.ws = ws;
    ws.addEventListener('open', () => { net.connected = true; ws.send(JSON.stringify({ t: 'join', name, code: net.code, token: net.token, acct, ship, weapon, skin })); });
