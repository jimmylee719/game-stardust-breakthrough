...
選單與 Esc 選單都有「畫面風格」列，目前五種：霓虹（預設）、夕陽、水墨、像素、糖果。每位玩家各自選，存在瀏覽器裡。

實作方式（`public/js/themes.js`）：程式碼裡的來源色盤固定不動，主題只提供一張「來源色 → 目標色」對照表，
渲染用的 Canvas context 被包一層 Proxy，所有 `fillStyle / strokeStyle / shadowColor` 先經過對照表，
並可縮放光暈（`glow`）、線寬（`lineMul`）、替換字型（`font`）、降低內部解析度做像素風（`pixelScale`）。
HTML 覆蓋層透過 CSS 變數同步換色。伺服器與遊戲邏輯完全不用改。

新增主題：在 `THEMES` 加一個物件即可；對照表的鍵值列在檔案開頭的註解。
之後若要用圖片精靈，也是掛在同一個主題物件上（例如 `sprites: {...}`），渲染時有圖用圖、沒圖畫向量。
